 ...
 */
function verifySchoolAdmin(request, schoolId) {
    if (!request.auth) {
 ...
 */
exports.createSchool = functions.https.onCall(async (request) => {
    // Check auth and role
 ...
 * Must be called by a Super Admin.
 */
exports.toggleSchoolStatus = functions.https.onCall(async (request) => {
    // Check auth and role
    if (!request.auth || request.auth.token.role !== 'super_admin') {
        throw new functions.https.HttpsError('failed-precondition', 'The function must be called by an authenticated super admin.');
    }
    ...
    if (!schoolId || typeof disabled !== 'boolean') {
        throw new functions.https.HttpsError('invalid-argument', 'The function must be called with schoolId and disabled status (boolean).');
    }
    try {
        const db = admin.firestore();
 ...
 */
exports.createTeacher = functions.https.onCall(async (request) => {
 ...
 */
exports.updateTeacher = functions.https.onCall(async (request) => {
 ...
 */
exports.generateTempPassword = functions.https.onCall(async (request) => {
 ...
 */
exports.softDeleteUser = functions.https.onCall(async (request) => {
 ...
 */
exports.restoreUser = functions.https.onCall(async (request) => {
    const { schoolId, collectionType, docId } = request.data || {};
    if (!schoolId || !collectionType || !docId) {
        throw new functions.https.HttpsError('invalid-argument', 'Parameter tidak lengkap.');
    }
 ...
 */
exports.importStudentsBulk = functions.https.onCall(async (request) => {
 ...
 */
exports.resetStudentSession = functions.https.onCall(async (request) => {
...
function runAllocationAlgorithm(students, rooms, mode, options) {
    let pool = [...students];
    const warnings = [];
    if (mode === 'random') {
...
async function fetchActiveRooms(db, schoolId) {
...
async function deleteQueryInChunks(db, query) {
    if (query.empty)
        return;
    let batch = db.batch();
    let count = 0;
...
exports.executeAllocation = functions.https.onCall(async (request) => {
    const { schoolId, eventId, mode, options } = request.data || {};
    if (!schoolId || !eventId || !mode || !options) {
        throw new functions.https.HttpsError('invalid-argument', 'Parameter tidak lengkap.');
    }
...
exports.generateParticipantNumbers = functions.https.onCall(async (request) => {
    ...
    for (const doc of seatsSnap.docs) {
        ...
        batch.update(doc.ref, { participantNumber: num, roomCode: roomCodeClean, updatedAt: firestore_1.FieldValue.serverTimestamp() });
        count++;
        if (count === 500) {
            await batch.commit();
            batch = db.batch();
...
exports.exportRoomList = functions.https.onCall(async (request) => {
    ...
    verifySchoolAdmin(request, schoolId);
    const db = admin.firestore();
    let query = db.collection('schools').doc(schoolId).collection('events').doc(eventId)
        .collection('allocations').doc(allocationId).collection('seats');
    if (roomId) {
...
exports.assignProctors = functions.https.onCall(async (request) => {
...
exports.rescheduleSession = functions.https.onCall(async (request) => {
...
exports.rollbackAllocation = functions.https.onCall(async (request) => {
